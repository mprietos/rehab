...
psql -U postgres -d adiccare_db -f api/schema.sql
```
...
psql -U postgres -d adiccare_db -f api/seed.sql
```
...
# Ejecutar schema
docker exec -i rehabcompanion-db psql -U postgres -d adiccare_db < api/schema.sql

# Ejecutar seed
docker exec -i rehabcompanion-db psql -U postgres -d adiccare_db < api/seed.sql
```
...
# DATABASE_URL=postgresql://TU_USUARIO:TU_PASSWORD@localhost:5432/adiccare_db
...
```bash
# Crear backup
pg_dump -U postgres adiccare_db > backup.sql

# Restaurar desde backup
psql -U postgres -d adiccare_db < backup.sql
```
